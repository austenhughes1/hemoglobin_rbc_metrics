const firebaseConfig = {
  apiKey:            "AIzaSyAZ3QVcn7vuqP4fb1NfqhN3d4QQ0XwTdE0",
  authDomain:        "hemoglobin-rbc-metrics.firebaseapp.com",
  projectId:         "hemoglobin-rbc-metrics",
  storageBucket:     "hemoglobin-rbc-metrics.appspot.com", // note .appspot.com
  messagingSenderId: "393224344418",
  appId:             "1:393224344418:web:ec82c723c26a797fae123b"
};

// Initialize Firebase (namespaced v8 API)
firebase.initializeApp(firebaseConfig);

// (Optionally) if you want analytics, load firebase-analytics.js in your HTML
// and then call:
// firebase.analytics();