...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnGuest.addEventListener('click', () => {
    // Guest: go straight to data entry
  ...
  auth.onAuthStateChanged(user => {
    ...
    // Logged in: fetch saved formula
    db.collection('formulas').doc(user.uid).get()
      .then(doc => {
        if (doc.exists) {
          // We have saved coefficients → use them
          const d = doc.data();
          Object.assign(window._coeffRbc, d.coeffRbc);
          Object.assign(window._coeffHct, d.coeffHct);
          // Show custom converter immediately
          showOnly(customCalculator);
        } else {
          // No saved formula yet → prompt for data
          showOnly(dataSection);
        }
      })
      .catch(err => {
        console.error('Error loading formula:', err);
        // On error, fall back to data entry
        showOnly(dataSection);
      });
  });
  ...
  btnGenerate.addEventListener('click', () => {
    ...
    // Compute 1-variable fit (folding MCV into intercept)
    const n = hgb.length;
    ...
    // Reveal custom converter and hide data entry
    showOnly(customCalculator);
  });
});
